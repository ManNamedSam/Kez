...
fn defineStringMethod(name: []const u8, function: obj.ObjectMethodFn) !void {
    ...
    vm.push(Value.makeObj(@ptrCast(obj.ObjNativeMethod.init(function, obj.ObjType.String))));
...
pub fn lengthStringMethod(object: *Obj, arg_count: u8, args: [*]Value) Value {
    ...
    return Value.makeNumber(@floatFromInt(string.chars.len - 1));
}
...
pub fn sliceStringMethod(object: *Obj, arg_count: u8, args: [*]Value) Value {
    ...
    const new_string = obj.ObjString.copy(new_string_chars.ptr, new_string_chars.len);
